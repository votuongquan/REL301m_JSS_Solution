...
def log(message: str, color: str = Colors.WHITE, prefix: str = "INFO"):
    ...
    print(
        f"{Colors.BOLD}[{timestamp}]{Colors.END} {color}{prefix}:{Colors.END} {message}"
    )
...
class JSSFileService:
    ...
    def __init__(
        self, instances_dir: str, controllers_dir: str, results_dir: str = None
    ):
        ...
        if self.controllers_dir.exists():
            for file_path in self.controllers_dir.iterdir():
                if file_path.is_file() and file_path.suffix == ".txt":
                    ...
                    controller_info = self._parse_controller_file(
                        file_path, include_stats
                    )
    ...
    def _parse_instance_file(
        self, file_path: Path, include_stats: bool = False
    ) -> InstanceInfo:
        ...
        try:
            ...
            with open(file_path, "r") as f:
                ...
                if include_stats and qualifications_per_person:
                    ...
                    avg_qualifications = sum(qualifications_per_person) / len(
                        qualifications_per_person
                    )
                    max_possible_machines = max(machines) if machines else 1
                    coverage_percentage = (
                        (num_machines / max_possible_machines) * 100
...
class JSSExecutionService:
    ...
    async def run_comparison(self, request: ComparisonRequest) -> ComparisonResult:
        ...
        if request.controller_name:
            ...
            controller_path = self.file_service.get_controller_path(
    ...
    async def _run_comparison_background_task(
        self, task_id: str, request: ComparisonRequest
    ):
        ...
        try:
            ...
            if request.controller_name:
                if not self.file_service.controller_exists(request.controller_name):
                    raise ValueError(
                        f"Controller '{request.controller_name}' not found"
                    )
                controller_path = self.file_service.get_controller_path(
                    request.controller_name
                )
    ...
    async def generate_visualizations(
        ...
        try:
            ...
            if "gantt" in request.visualization_types:
                ...
                gantt_paths = await self._generate_gantt_charts(
                    request, gantt_dir, request.format, None
                )
    ...
    async def _generate_visualizations_for_result(
        self, result: ComparisonResult, task_id: str
    ):
        ...
        try:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            results_subdir = (
                self.file_service.results_dir / f"{result.instance_name}_{timestamp}"
            )
            results_subdir.mkdir(exist_ok=True)
    ...
    def _create_agent(
        ...
        if agent_type == AgentType.HYBRID:
            ...
            if not all([instance_path, controller_path, num_people]):
                ...
                raise ValueError(
                    "Controller agent requires instance_path, controller_path, and num_people"
                )
    ...
    def _run_comparison_sync(
        ...
        for agent_type in request.agents:
            if agent_type == AgentType.CONTROLLER and controller_path:
                ...
                controller_info = next(
                    (
                        c
                        for c in self.file_service.get_controllers()
                        if c.name == request.controller_name
                    ),
                    None,
                )
                if controller_info:
        ...
        results_df = framework.run_comprehensive_comparison(
            custom_agents=custom_agents, num_episodes=request.num_episodes
        )
    ...
    async def run_single_episode(self, request: SingleRunRequest) -> SingleRunResult:
        ...
        if not self.file_service.instance_exists(request.instance_name):
            log(
                f"❌ Instance not found: {request.instance_name}", Colors.RED, "EPISODE"
            )
            raise ValueError(f"Instance '{request.instance_name}' not found")
    ...
    def _run_single_episode_sync(
        ...
        if request.agent_type == AgentType.CONTROLLER:
            ...
            log(
                f"  🏃 Running episode with schedule capture...", Colors.CYAN, "EPISODE"
            )
            makespan, total_reward, execution_time, schedule = (
                framework.env_manager.run_episode_with_schedule_capture(
                    lambda env, obs: agent(env, obs)
                )
            )
