...
class JSSStreamingService:
	...
	async def start_comparison_stream(self, request: StreamComparisonRequest) -> str:
		...
		task = asyncio.create_task(self._run_comparison_background(session_id, request))
	...
	async def _run_comparison_background(self, session_id: str, request: StreamComparisonRequest):
		...
		try:
			...
			if not env_test_success:
				await self.stream_manager.send_error(session_id, 'Failed to create JSS environment', {'instance_path': instance_path})
				return
			...
			for agent_type in request.agents:
				try:
					if agent_type == StreamAgentType.CONTROLLER:
						...
						agent = create_stream_agent(agent_type.value, controller_path=controller_path, num_people=request.num_people)
					elif agent_type == StreamAgentType.LOOKAHEAD:
						agent = create_stream_agent(agent_type.value, lookahead_depth=request.lookahead_depth or 2)
					else:
					...
					await self.stream_manager.send_error(session_id, f'Failed to create agent {agent_type}', {'error': str(e)})
			...
			async def stream_callback(event_type: str, **kwargs):
				try:
					if event_type == 'comparison_start':
						...
						await self.stream_manager.broadcast_to_session_subscribers(session_id, {'type': 'comparison_start', 'session_id': session_id, 'total_methods': kwargs.get('total_methods', 0), 'total_episodes_per_method': kwargs.get('total_episodes_per_method', 0), 'timestamp': datetime.now().isoformat()})

					elif event_type == 'method_start':
						await self.stream_manager.broadcast_to_session_subscribers(session_id, {'type': 'method_start', 'session_id': session_id, 'method_name': kwargs.get('method_name'), 'method_type': kwargs.get('method_type'), 'completed_methods': kwargs.get('completed_methods', 0), 'total_methods': kwargs.get('total_methods', 0), 'timestamp': datetime.now().isoformat()})

					elif event_type == 'episode_start':
						await self.stream_manager.send_episode_start(session_id, kwargs.get('episode', 0), kwargs.get('agent_name', 'Unknown'))
						...
						if step % 5 == 0:  # Every 5 steps
							await self.stream_manager.send_episode_progress(session_id, kwargs.get('episode', 0), kwargs.get('agent_name', 'Unknown'), step, kwargs.get('action', 0), kwargs.get('reward', 0), kwargs.get('current_makespan'))

					elif event_type == 'episode_complete':
						await self.stream_manager.send_episode_complete(session_id, kwargs.get('episode', 0), kwargs.get('agent_name', 'Unknown'), kwargs.get('makespan', 0), kwargs.get('total_reward', 0), kwargs.get('execution_time', 0))

					elif event_type == 'method_complete':
						await self.stream_manager.broadcast_to_session_subscribers(session_id, {'type': 'method_complete', 'session_id': session_id, 'method_name': kwargs.get('method_name'), 'results': kwargs.get('results', {}), 'completed_methods': kwargs.get('completed_methods', 0), 'total_methods': kwargs.get('total_methods', 0), 'timestamp': datetime.now().isoformat()})
						...
						await self.stream_manager.send_error(session_id, kwargs.get('error_message', 'Unknown error'), kwargs)

				except Exception as e:
					logger.error(f'Error in stream callback: {e}')

			# Run comprehensive comparison
			results = await framework.run_comprehensive_comparison_stream(custom_agents=agents, dispatching_rules=[rule.value for rule in request.dispatching_rules], num_episodes=request.num_episodes, include_random=request.include_random_baseline, stream_callback=stream_callback)

			# Update final session status
			await self.stream_manager.update_session_status(session_id, 'completed', results=results)

		except Exception as e:
			logger.error(f'Error in comparison stream {session_id}: {e}')
			await self.stream_manager.send_error(session_id, f'Comparison failed: {str(e)}', {'traceback': str(e)})
			await self.stream_manager.update_session_status(session_id, 'failed', error_message=str(e))
	...
	async def cancel_session(self, session_id: str) -> bool:
		...
		if session_id in self.active_tasks:
			...
			await self.stream_manager.update_session_status(session_id, 'cancelled')
	...
	async def get_streaming_stats(self) -> StreamStats:
		...
		return StreamStats(total_sessions=len(all_sessions), active_sessions=active_sessions, total_connections=self.stream_manager.get_connection_count())
	...
	async def convert_results_to_stream_format(self, results: Dict) -> StreamResults:
		...
		for method_name, method_data in results.items():
			...
			method_result = StreamMethodResult(method_name=method_name, method_type=self._get_method_type(method_name), episodes_completed=method_data.get('episodes_completed', 0), avg_makespan=method_data.get('avg_makespan', 0), best_makespan=method_data.get('best_makespan', 0), worst_makespan=method_data.get('worst_makespan', 0), std_makespan=method_data.get('std_makespan', 0), avg_reward=method_data.get('avg_reward', 0), avg_execution_time=method_data.get('avg_execution_time', 0))
