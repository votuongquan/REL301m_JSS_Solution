		...
		while True:
			try:
				...
				if connection and connection.is_active:
					await stream_manager.send_to_client(client_id, {'type': 'error', 'error_message': 'Invalid JSON format', 'timestamp': datetime.now().isoformat()})

			except Exception as e:
				logger.error(f'Error handling message from {client_id}: {e}')
				# Send error if connection is still active
				if connection and connection.is_active:
					await stream_manager.send_to_client(client_id, {'type': 'error', 'error_message': 'Internal server error', 'timestamp': datetime.now().isoformat()})
				break
...
async def start_streaming_comparison(request: StreamComparisonRequest, service: JSSStreamingService = Depends(get_streaming_service)):
	"""Start a new streaming comparison"""
	try:
		session_id = await service.start_comparison_stream(request)

		return {'success': True, 'session_id': session_id, 'message': 'Streaming comparison started', 'websocket_url': f'/api/v1/stream/ws', 'subscribe_message': {'type': 'subscribe', 'session_id': session_id}}
...
async def get_streaming_sessions(service: JSSStreamingService = Depends(get_streaming_service)):
...
async def get_streaming_session(session_id: str, service: JSSStreamingService = Depends(get_streaming_service)):
...
async def cancel_streaming_session(session_id: str, service: JSSStreamingService = Depends(get_streaming_service)):
...
async def get_streaming_stats(service: JSSStreamingService = Depends(get_streaming_service)):
...
async def stream_health_check(service: JSSStreamingService = Depends(get_streaming_service)):
	"""Health check for streaming service"""
	try:
		stats = await service.get_streaming_stats()

		return HealthCheck(status='healthy', timestamp=datetime.now(), active_connections=stats.total_connections, active_sessions=stats.active_sessions, uptime_seconds=service.get_uptime())

	except Exception as e:
		logger.error(f'Health check failed: {e}')
		return HealthCheck(status='unhealthy', timestamp=datetime.now(), active_connections=0, active_sessions=0, uptime_seconds=0)


@router.post('/cleanup')
async def cleanup_streaming_service(service: JSSStreamingService = Depends(get_streaming_service)):
...
class JSSStreamingClient {
    constructor(wsUrl = 'ws://localhost:8081/api/v1/stream/ws') {
        this.wsUrl = wsUrl;
        this.ws = null;
        this.clientId = null;
...
"""
	...
	return JSONResponse(content={'example_code': example_code}, headers={'Content-Type': 'application/json'})
