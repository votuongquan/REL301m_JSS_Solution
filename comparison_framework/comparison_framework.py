            ...
            if action < env.jobs:
                ...
                if current_op < env.machines:
                    ...
                    actual_start_time = max(
                        current_time, current_time + machine_available_time)
                    actual_end_time = actual_start_time + proc_time

                    schedule.append(
                        (job_id, machine_id, actual_start_time, actual_end_time))
...
class JSSComparisonFramework:
    ...
    def run_agent_evaluation(self, agent: BaseJSSAgent, num_episodes: int = 10) -> Dict[str, float]:
        ...
        for episode in range(num_episodes):
            ...
            makespan, total_reward, execution_time = self.env_manager.run_episode(
                agent_wrapper, env)
    ...
    def run_comprehensive_comparison(self, custom_agents: List[BaseJSSAgent],
        ...
        for agent in custom_agents:
            ...
            self.results[agent.get_name()] = self.run_agent_evaluation(
                agent, num_episodes)
        ...
        for rule in self.dispatching_rules:
            ...
            try:
                self.results[rule] = self.run_dispatching_rule_evaluation(
                    rule, num_episodes)
    ...
    def print_summary(self):
        ...
        print(
            f"{'Rank':<4} {'Method':<20} {'Avg Makespan':<12} {'Min Makespan':<12} {'Avg Reward':<12}")
        print("-" * 70)
    ...
    def create_enhanced_visualizations(self, save_dir: str = "results", custom_agents: List[BaseJSSAgent] = None):
        ...
        if custom_agents:
            ...
            gantt_data = self.run_custom_agents_with_gantt(
                custom_agents, save_dir)
    ...
    def _generate_performance_report(self, save_dir: str):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write(
                f"   Standard Deviation: {best_stats['std_makespan']:.1f}\n")
            f.write(
                f"   Best Single Result: {best_stats['min_makespan']:.1f}\n")
            f.write(
                f"   Consistency Score: {best_stats['std_makespan']/best_stats['avg_makespan']*100:.1f}%\n")
            ...
            if custom_agents:
                ...
                for agent in custom_agents:
                    ...
                    rank = [i for i, (m, _) in enumerate(
                        sorted_methods, 1) if m == agent][0]
                    f.write(f"{agent}:\n")
                    f.write(f"   Rank: #{rank} out of {len(self.results)}\n")
                    f.write(
                        f"   Performance: {stats['avg_makespan']:.1f} ± {stats['std_makespan']:.1f}\n")
    ...
    def run_custom_agents_with_gantt(self, custom_agents: List[BaseJSSAgent], save_dir: str = "results") -> Dict[str, List]:
        ...
        for agent in custom_agents:
            ...
            for episode in range(5):  # Run 5 episodes to find the best one
                ...
                makespan, _, _, schedule = self.env_manager.run_episode_with_schedule_capture(
                    agent_wrapper, env)
