...
def main():
	...
	parser = argparse.ArgumentParser(description='Job Shop Scheduling Comparison Framework')
	parser.add_argument('--instance', type=str, default='dmu16', help='Instance name (default: dmu16)')
	parser.add_argument(
		'--episodes',
		type=int,
		default=30,
	...
	try:
		...
		custom_results = {name: framework.results[name] for name in custom_agents if name in framework.results}

		if custom_results:
			best_custom = min(custom_results.items(), key=lambda x: x[1]['avg_makespan'])
			...
			rule_results = {name: data for name, data in framework.results.items() if name in framework.dispatching_rules}

			if rule_results:
				best_rule = min(rule_results.items(), key=lambda x: x[1]['avg_makespan'])
				improvement = (best_rule[1]['avg_makespan'] - best_custom[1]['avg_makespan']) / best_rule[1]['avg_makespan'] * 100

				print(f'🎲 Best dispatching rule: {best_rule[0]} (makespan: {best_rule[1]["avg_makespan"]:.2f})')

				if improvement > 0:
					print(f'🚀 Custom agent improvement: {improvement:.2f}% better than best rule!')
				else:
					print(f'📉 Custom agent performance: {abs(improvement):.2f}% worse than best rule')
