...
class AdvancedJSSVisualizer:
    ...
    def _create_performance_overview(self, ax):
        ...
        avg_makespans = [self.results[method]['avg_makespan']
                         for method in methods]
        std_makespans = [self.results[method]['std_makespan']
                         for method in methods]

        # Sort by performance
        sorted_data = sorted(
            zip(methods, avg_makespans, std_makespans), key=lambda x: x[1])
    ...
    def _create_ranking_plot(self, ax):
        ...
        sorted_methods = sorted(
            zip(methods, avg_makespans), key=lambda x: x[1])
        ...
        bars = ax.barh(ranks, [m[1] for m in sorted_methods],
                       color=[self.colors.get(m, '#696969')
        ...
        ax.set_yticks(ranks)
        ax.set_yticklabels(
            [f"{i}. {name}" for i, name in enumerate(method_names, 1)])
    ...
    def _create_scatter_plot(self, ax):
        ...
        ax.set_xlabel('Average Makespan (Performance)',
                      fontsize=10, fontweight='bold')
        ax.set_ylabel('Standard Deviation (Consistency)',
                      fontsize=10, fontweight='bold')
        ...
        if avg_makespans and std_makespans:
            ...
            ax.fill_betweenx([0, avg_y], 0, avg_x, alpha=0.1,
                             color='green', label='Best Zone')
        ...
        for col in stats_df.columns:
            ...
            if col_max != col_min:
                stats_normalized[col] = (
                    stats_df[col] - col_min) / (col_max - col_min)
            else:
    ...
    def _create_summary_table(self, ax):
        ...
        methods = list(self.results.keys())
        sorted_methods = sorted(
            methods, key=lambda x: self.results[x]['avg_makespan'])
        ...
        for i, method in enumerate(sorted_methods[:5]):  # Top 5
            ...
            rank_symbols = ["WINNER", "RUNNER-UP",
                            "3RD PLACE", "4TH PLACE", "5TH PLACE"]
            rank_text = rank_symbols[i] if i < len(
                rank_symbols) else f"{i+1}TH PLACE"
        ...
        table = ax.table(cellText=table_data,
                         colLabels=['Rank & Method', 'Avg Makespan',
                                    'Std Dev', 'Best Result', 'Avg Time'],
                         cellLoc='center',
                         loc='center',
    ...
    def _create_cumulative_performance(self, ax):
        ...
        for method in self.results.keys():
            ...
            cumulative_avg = np.cumsum(
                makespans) / np.arange(1, len(makespans) + 1)
            episodes = range(1, len(makespans) + 1)
    ...
    def _create_statistical_analysis(self, ax):
        ...
        ax.set_title(
            'Statistical Analysis (95% Confidence Intervals)', fontweight='bold')
        ax.grid(True, alpha=0.3)
    ...
    def create_gantt_charts_for_agents(self, gantt_data: Dict, save_dir: str = "results"):
        ...
        for agent_name, data in gantt_data.items():
            ...
            self._create_single_gantt_chart(
                agent_name, schedule, makespan, str(gantt_dir))
