...
class StreamEnvironmentManager:
	...
	async def run_episode_stream(self, agent_function: Callable, env=None, stream_callback: Callable = None) -> Tuple[float, float, float]:
		"""
		...
		while not done:
			...
			# Stream progress
			if stream_callback:
				await stream_callback('step', step=step, action=action, reward=reward, done=done, obs=obs, total_reward=total_reward, current_makespan=getattr(env, 'current_time_step', 0))
...
class StreamJSSFramework:
	...
	async def run_agent_evaluation_stream(self, agent: BaseStreamJSSAgent, num_episodes: int = 10, stream_callback: Callable = None) -> Dict[str, float]:
		...
		for episode in range(num_episodes):
			...
			# Send episode start event
			if stream_callback:
				await stream_callback('episode_start', episode=episode + 1, total_episodes=num_episodes, agent_name=agent_name)

			# Create episode-specific callback
			async def episode_callback(event_type, **kwargs):
				if stream_callback:
					await stream_callback('episode_progress', episode=episode + 1, agent_name=agent_name, **kwargs)

			# Run episode
			try:
				makespan, total_reward, execution_time = await self.env_manager.run_episode_stream(agent, None, episode_callback)

				metrics.add_episode_result(makespan, total_reward, execution_time)

				# Send episode complete event
				if stream_callback:
					await stream_callback('episode_complete', episode=episode + 1, agent_name=agent_name, makespan=makespan, total_reward=total_reward, execution_time=execution_time, current_stats=metrics.get_current_stats())

			except Exception as e:
				if stream_callback:
					await stream_callback('error', episode=episode + 1, agent_name=agent_name, error_message=str(e))
				break

		return metrics.get_current_stats()

	async def run_dispatching_rule_evaluation_stream(self, rule_name: str, num_episodes: int = 10, stream_callback: Callable = None) -> Dict[str, float]:
	...
	async def run_random_baseline_stream(self, num_episodes: int = 10, stream_callback: Callable = None) -> Dict[str, float]:
	...
	async def run_comprehensive_comparison_stream(self, custom_agents: List[BaseStreamJSSAgent], dispatching_rules: List[str] = None, num_episodes: int = 10, include_random: bool = True, stream_callback: Callable = None) -> Dict[str, Dict]:
		"""
		...
		if stream_callback:
			await stream_callback('comparison_start', total_methods=total_methods, total_episodes_per_method=num_episodes)
		...
		for method_type, method_obj in all_methods:
			...
			try:
				if method_type == 'agent':
					...
					if stream_callback:
						await stream_callback('method_start', method_name=method_name, method_type='agent', completed_methods=completed_methods, total_methods=total_methods)
					...
					if stream_callback:
						await stream_callback('method_start', method_name=method_name, method_type='dispatching_rule', completed_methods=completed_methods, total_methods=total_methods)
					...
					if stream_callback:
						await stream_callback('method_start', method_name=method_name, method_type='random', completed_methods=completed_methods, total_methods=total_methods)
				...
				if stream_callback:
					await stream_callback('method_complete', method_name=method_name, results=results, completed_methods=completed_methods, total_methods=total_methods)

			except Exception as e:
				# Send error event
				if stream_callback:
					await stream_callback('error', method_name=method_name or 'Unknown', error_message=str(e), completed_methods=completed_methods, total_methods=total_methods)
		...
		if stream_callback:
			await stream_callback('comparison_complete', results=self.results, total_methods=total_methods)
	...
	def get_results_summary(self) -> Dict:
		...
		for method_name, results in self.results.items():
			...
			summary[method_name] = {'avg_makespan': avg_makespan, 'best_makespan': results.get('best_makespan', 0), 'episodes_completed': results.get('episodes_completed', 0), 'avg_execution_time': results.get('avg_execution_time', 0)}

		summary['_meta'] = {'best_method': best_method, 'best_makespan': best_makespan, 'total_methods': len(self.results)}
