...
class StreamHybridPriorityScoringAgent(BaseStreamJSSAgent):
	...
	def _work_remaining_score(self, env, job_idx: int) -> float:
		...
		current_op = env.todo_time_step_job[job_idx]
		remaining_work = sum(env.instance_matrix[job_idx][op][1] for op in range(current_op, env.machines))
	...
	def _machine_utilization_score(self, env, machine_id: int) -> float:
		...
		machine_load = sum(1 for job in range(env.jobs) if env.needed_machine_jobs[job] == machine_id and env.todo_time_step_job[job] < env.machines)
	...
	def _bottleneck_machine_score(self, env, job_idx: int, current_op: int) -> float:
		...
		total_machine_load = sum(env.instance_matrix[j][op][1] for j in range(env.jobs) for op in range(env.machines) if env.instance_matrix[j][op][0] == current_machine)
	...
	def _get_dynamic_weights(self, progress_ratio: float, env) -> Dict[str, float]:
		...
		if progress_ratio < 0.3:
			return {'spt': 0.4, 'work_remaining': 0.3, 'critical_path': 0.1, 'machine_util': 0.1, 'bottleneck': 0.05, 'flow_continuity': 0.05}

		# Middle stage: balanced approach
		elif progress_ratio < 0.7:
			return {'spt': 0.25, 'work_remaining': 0.25, 'critical_path': 0.2, 'machine_util': 0.15, 'bottleneck': 0.1, 'flow_continuity': 0.05}

		# Late stage: focus on critical path and bottlenecks
		else:
			return {'spt': 0.15, 'work_remaining': 0.15, 'critical_path': 0.3, 'machine_util': 0.2, 'bottleneck': 0.15, 'flow_continuity': 0.05}
...
class StreamAdaptiveLookAheadAgent(BaseStreamJSSAgent):
	...
	def _estimate_future_reward(self, env) -> float:
		...
		total_remaining = sum(sum(env.instance_matrix[job][op][1] for op in range(env.todo_time_step_job[job], env.machines)) for job in range(env.jobs))
...
class StreamControllerJSSAgent(BaseStreamJSSAgent):
		...
		try:
			...
			if lines:
				...
				if len(first_line) >= 2:
					...
					self.controller_data = {'total_people': total_people, 'total_machines': total_machines, 'people_to_use': people_to_use, 'qualifications': []}
	...
	def _controller_selection(self, env, legal_job_indices: List[int]) -> int:
		...
		for job_idx in legal_job_indices:
			...
			qualified_people = sum(1 for person_machines in self.controller_data['qualifications'] if required_machine in person_machines)
